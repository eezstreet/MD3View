...
{
	char texname[2048];
	char texnameJPG[2048];
	char texnamePNG[2048];
	char texpath[2048];

	bool bHaveWeFoundItYet = false;

	// mate if it has a different extension don't allow for it, plain and simple
	string s = texturePath;
	auto period = s.find_last_of('.');
	string ext = s.substr(period);
	if(!ext.compare(".tif") || !ext.compare(".psd") || !ext.compare(".tiff"))
		return false;
	...
	if(file_exists(texname) || file_exists(texnameJPG) || file_exists(texnamePNG))
		bHaveWeFoundItYet = true;

	// FORWARDS SEARCH
	if(!bHaveWeFoundItYet) {
	...
	// BACKWARDS SEARCH
	if(!bHaveWeFoundItYet) { // Yeah, I was afraid of this. I guess do the same as above, but from the opposite direction
		char tempPath[1024];
		strcpy( texpath, (const char *)mdview.basepath );
		strcpy( texname, Filename_WithoutExt(texturePath) );
		strlwr( texname );
	...
	//=========
		int k = 1;
		int doNotGoAboveThis = strlen(texturePath);
		do {
			k++;
			while((k<doNotGoAboveThis) && (texturePath[k]!='\\') && (texturePath[k]!='/') )
				k++;
			strcpy(tempPath, texpath);
			strcat(tempPath, texturePath+k);
			if (k<doNotGoAboveThis) 
			{
				strcpy( texname, Filename_WithoutExt(tempPath) );
				strlwr( texname );
				strcat( texname, ".tga" );
...
//=========
		...
		} while ( (k<doNotGoAboveThis) && (!file_exists(texname) && !file_exists(texnameJPG) && !file_exists(texnamePNG)) );

		if(k < doNotGoAboveThis)
			bHaveWeFoundItYet = true;
	}

	if(!bHaveWeFoundItYet) { // Hmm, still haven't found it? should probably just not load it
