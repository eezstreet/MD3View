...
// COOL NEW SPECIAL THING THAT LOADS FBX FILES
//
...
bool loadfbx( char *filename )
{
    ...
    ptManager = FbxManager::Create();

	// create an IOSettings object
	FbxIOSettings * ios = FbxIOSettings::Create(ptManager, "IOSRoot" );
	ptManager->SetIOSettings(ios);

	FbxScene* scene = FbxScene::Create(ptManager, "");
    ...
    // Create an importer.
    FbxImporter* lImporter = FbxImporter::Create(ptManager,"");
    
    // Initialize the importer by providing a filename.
    const bool lImportStatus = lImporter->Initialize(filename, -1, ptManager->GetIOSettings() );
	...
	if(!lStatus)
		return false; // No password protected files allowed

	// Destroy the importer
    lImporter->Destroy();

	// Right, let's fill in the appropriate stuff for our model then.
	gl_model* model = new gl_model; memset( model, 0, sizeof(gl_model) ); // FIXME: UNSAFE AS ALL HELL
	strcpy(model->sHeadSkinName, "default");
	strcpy(model->sMDXFullPathname, filename);
	strcpy(model->sMD3BaseName, Filename_WithoutPath(filename));
	
	string folder = filename;
	auto lastSlash = folder.find_last_of("\\/");
	if(lastSlash != folder.npos) {
		folder = folder.substr(0, lastSlash);
		strcpy(mdview.basepath, folder.c_str());
	}
	...
	int iNumNodes = scene->GetNodeCount();
	int *nodeIndices = new int[scene->GetNodeCount()];
	int iNumUsedNodes = 0;

	// Figure out which nodes we want to use.
	for(int i = 0; i < iNumNodes; i++) {
		auto x = scene->GetNode(i);
		if(!stricmp(x->GetName(), "RootNode"))
			continue; // ignore
		if(stricmp(x->GetTypeName(), "Mesh"))
			continue; // not a mesh. we don't care
		nodeIndices[iNumUsedNodes++] = i;
	}

	model->iNumMeshes = iNumUsedNodes;
	model->pMeshes = new gl_mesh[model->iNumMeshes];
	model->modelType = MODTYPE_MD3;

	// onto the actual gravy of the func -- load the meshes and stuff
	for(int i = 0; i < iNumUsedNodes; i++) {
		auto x = scene->GetNode(nodeIndices[i]);
		auto mesh = x->GetMesh();
		auto m = &model->pMeshes[i];
		FbxAMatrix globalMatrix = scene->GetAnimationEvaluator()->GetNodeGlobalTransform (x);

		strcpy(model->pMeshes[i].sName, x->GetName());

		// Init some stuff for UVs
		FbxStringList uvNameSetList;
		mesh->GetUVSetNames(uvNameSetList);
		if ( uvNameSetList.GetCount() == 0 )
			return false;
		const char *uvNameSet = uvNameSetList[0];

		m->iNumVertices = mesh->GetPolygonVertexCount();
		m->iNumTriangles = mesh->GetPolygonCount();
		m->iNumMeshFrames = 1; // NOT PROCESSED YET
		m->iNumSkins = x->GetSrcObjectCount<FbxSurfaceMaterial>();
		
		m->meshFrames = new FMeshFrame[m->iNumMeshFrames];
		m->triangles = new TriVec[m->iNumTriangles];
		m->textureCoord = new TexVec[m->iNumVertices];
		m->iterMesh = new Vec3[m->iNumVertices];
		m->bindings = new GLuint[m->iNumSkins];

		// hack - animation is currently not supported
		m->meshFrames[0].pVerts = new Vec3[m->iNumVertices];
		m->meshFrames[0].pNormalIndexes = new short[m->iNumVertices];

		// Get the vertices and their texture coordinates.
		auto positions = mesh->GetControlPoints();
		for(int j = 0; j < m->iNumTriangles; j++) {
			m->triangles[j][0] = mesh->GetPolygonVertex(j, 0);
			m->triangles[j][1] = mesh->GetPolygonVertex(j, 1);
			m->triangles[j][2] = mesh->GetPolygonVertex(j, 2);
			
			// By getting the vertex position as well as texture coords in the same loop, we cut loading time by 1/2
			for(int k = 0; k < 3; k++) {
				FbxVector2 texcoord;
				bool noTexCoord = false;
				mesh->GetPolygonVertexUV(j, k, uvNameSet, texcoord, noTexCoord);
				float u = static_cast<float>(texcoord[0]);
				float v = static_cast<float>(texcoord[1]);
				m->textureCoord[m->triangles[j][k]][0] = u;
				m->textureCoord[m->triangles[j][k]][1] = 1.0f-v;

				FbxVector4 corrected = globalMatrix.MultT(positions[m->triangles[j][k]]);
				for(int l = 0; l < 3; l++) {
					m->meshFrames[0].pVerts[m->triangles[j][k]][l] = corrected[l];
				}
				m->meshFrames[0].pNormalIndexes[m->triangles[j][k]] = corrected[3];
			}

			m->meshFrames[0].pNormalIndexes[m->triangles[j][0]] = 0; // dont think its used but not sure
		}

		// handle textures
		for(int j = 0; j < m->iNumSkins; j++) {
			FbxSurfaceMaterial* srf = x->GetSrcObject<FbxSurfaceMaterial>(j);
			if(!srf)
				continue;
            ...
            FBXSDK_FOR_EACH_TEXTURE(lTextureIndex) {
				lProperty = srf->FindProperty(FbxLayerElement::sTextureChannelNames[lTextureIndex]);
				int textureCount = lProperty.GetSrcObjectCount<FbxTexture>();

				if(textureCount) {
					// This is stupid, we need to convert from layered texture to texture...weird I know.
					FbxLayeredTexture *lLayeredTexture = lProperty.GetSrcObject<FbxLayeredTexture>(0);
					FbxFileTexture* ftex;
					if(!lLayeredTexture) // Fall back to regular texture
					{
						FbxTexture* ftex2 = lProperty.GetSrcObject<FbxTexture>(0);
						if(!ftex2)
							continue;
						ftex = (FbxFileTexture*)ftex2;
					}
					else
...
bool loadmdl( char *filename )
{
	...
	LPCSTR psFBXEXT = strstr(String_ToLower(filename),".fbx");
	if(psFBXEXT)
	{
